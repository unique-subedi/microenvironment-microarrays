...
```{r Make adj R^2 tibble, eval = FALSE}
adj_r_sq_list = vector(mode = "list", length = ncol(lm_super_tbl_2_num))
for(i in seq_along(adj_r_sq_list)){
  var_name = colnames(lm_super_tbl_2_num)[[i]]
  lm_super_tbl_2 = super_tbl %>%
  mutate(ECM1 = as.factor(ECM1) %>% 
           relevel(ref = "COL1_go0005584")) %>%
  mutate(Ligand1 = as.factor(Ligand1) %>% 
           relevel(ref = "FBS_pubchemsid56314508")) %>%
    select(!(WellIndex:PinDiameter)) %>%
    select(where(is.numeric) | where(is.factor) | stain_num) %>%
    select(!PrintSpot & !ImageID & !ClarionID) %>%
    filter(!is.na(.[[var_name]]))
  lm_super_tbl_2_num = lm_super_tbl_2 %>% 
    select(where(is.numeric))
  if(length(unique(lm_super_tbl_2$stain_num)) == 1){
    adj_r_sq_list[[i]] = tibble(
      `Independent Variable` = var_name,
      simple_adj_r_sq = lm(lm_super_tbl_2_num[[i]] ~ 1, 
                       data = lm_super_tbl_2) %>% 
                       summary() %>% 
                       .[["adj.r.squared"]],
      adj_r_sq = lm(lm_super_tbl_2_num[[i]] ~ ECM1 + Ligand1 + 1, 
                       data = lm_super_tbl_2) %>% 
                       summary() %>% 
                       .[["adj.r.squared"]]
    )
    ...
    adj_r_sq_list[[i]] = tibble(
      `Independent Variable` = var_name,
      simple_adj_r_sq = lm(lm_super_tbl_2_num[[i]] ~ stain_num, 
                       data = lm_super_tbl_2) %>% 
                       summary() %>% 
                       .[["adj.r.squared"]],
      adj_r_sq = lm(lm_super_tbl_2_num[[i]] ~ ECM1 + Ligand1 + stain_num, 
                       data = lm_super_tbl_2) %>% 
                       summary() %>% 
                       .[["adj.r.squared"]]
    )
...
adj_r_sq_tibble = bind_rows(adj_r_sq_list) %>%
  mutate(Difference = adj_r_sq - simple_adj_r_sq) %>%
  filter(!is.na(Difference)) %>%
  arrange(desc(Difference)) %>%
  rename(`Adj R^2 (simple model)` = simple_adj_r_sq,
         `Adj R^2` = adj_r_sq)

# save(adj_r_sq_tibble, file = "adj_r_sq_tibble.rda")
```

```{r Load R^2 tibble to save time}
load("adj_r_sq_tibble.rda")

adj_r_sq_tibble %>%
  filter(`Adj R^2 (simple model)` != 0) %>%
  kable()

adj_r_sq_tibble %>%
  filter(`Adj R^2 (simple model)` == 0) %>%
  select(`Independent Variable`, `Adj R^2`) %>%
  kable()
```
