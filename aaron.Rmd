...
Simpler linear model (no interaction effects) has superior AIC. However, F-test suggests keeping interaction effects. For simplicity, I suggest leaving out interaction effects. 
...
area_treatment_lm = aov(
  Nuclei_CP_AreaShape_Area ~ ECM1*Ligand1+stain_num,
  data = lm_super_tbl
)
toc()
# 13.5 minutes
summary(area_treatment_lm)
AIC(area_treatment_lm)
simple_area_treatment_lm = aov(
  Nuclei_CP_AreaShape_Area ~ ECM1+Ligand1+stain_num,
  data = lm_super_tbl
)
summary(simple_area_treatment_lm)
AIC(simple_area_treatment_lm)
...
ggplot(simple_coefficients_tbl %>% 
    filter(Type == "ECM1"),
