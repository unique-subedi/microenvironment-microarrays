...
make_pca_results_plot <- function(pca_results, pattern = ".*", title) {
  ...
  panel3 <- make_first_2_pcs_loadings_plot(pca_results, ".*", title = NULL)
...
all_data2 %>%
  mutate(
    ...
    title = map_chr(
      ss_num, ~ str_interp("PCA Results for Staining Set ${.x}")
    ),
    pca_results_plot = map2(
      pca_results, title, make_pca_results_plot, pattern = ".*"
    )
  ) %>%
