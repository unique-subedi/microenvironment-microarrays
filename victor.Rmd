...
Discard columns that only have missing values.
```{r drop_na_only_cols}
drop_na_only_cols <- function(tbl) select(tbl, !where(~ all(is.na(.x))))
all_data <- drop_na_only_cols(all_data)
```

Create one tibble for each staining set.
```{r make_all_data2, cache=TRUE}
all_data2 <- all_data %>%
  nest(data = !StainingSet) %>%
  mutate(data = map(data, drop_na_only_cols))
```
...
all_data %>%
  ...
  summarize(num_plates = n_distinct(plate))
```
...
all_data2 %>%
  mutate(map(data, check_ECM2_cols_w_nas)) %>%
  pull()
```
...
all_data2 %>%
  mutate(map(data, check_Ligand1Lx)) %>%
  pull()
```
...
```{r nas_in_array_row_col, eval=FALSE}
data %>%
...
```{r spot_arrayrow_arraycol, eval=FALSE}
map_lgl(
  tbls,
  ~ .x %>%
...
```{r Spot_PA_SpotCellCount_SE, eval=FALSE}
map_lgl(
  tbls,
...
```{r Spot_PA_ReplicateCount, eval=FALSE}
map(
  tbls,
  . %>%
...
```{r one_plate_heatmap, eval=FALSE}
LI8X00510_Level2 %>%
  ggplot(aes(ArrayColumn, ArrayRow, fill = Spot_PA_SpotCellCount)) +
  facet_wrap(vars(Well), nrow = 2, labeller = label_both) +
  geom_tile() +
