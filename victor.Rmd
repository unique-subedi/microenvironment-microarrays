...
Define a function for calculating the medians and IQRs of all non-trivial numeric variables in a dataset whose names match a pattern. The output has one row for each variable and columns with the summary statistics. For each staining set, the five variables with the largest medians are also shown.
...
calc_summary_stats <- function(data, pattern = ".*") {
  data %>%
    ...
    summarize(
      across(.fns = list(median = median, iqr = IQR), .names = "{.col}|{.fn}")
    ) %>%
    pivot_longer(
...
make_summary_stats_plot <- function(summary_stats, title) {
  median_panel <- summary_stats %>%
    mutate(log_median = log10(median - min(median) + 1)) %>%
    ggplot(aes(log_median)) +
    geom_histogram(color = "black", fill = "lightgray", bins = 10) +
    labs(x = "Log Median", y = "Number of Variables") +
    theme_bw()
...
all_summary_stats %>%
  mutate(
    summary_stats = map(
      summary_stats, ~ slice(arrange(.x, desc(median)), n = 1:5)
    )
  ) %>%
...
The histograms below show the log-transformed medians and IQRs of the AreaShape variables. We again see that the variables differ in scale substantially. 
...
Looking only at the AreaShape variables, fewer than ten PCs account for over 95\% of the variation. In particular, for staining set 2, just four PCs account for that percentage of the variation.
```{r areashape_pca}
all_data2 %>%
  mutate(
    pca_results = map(data, run_pca, pattern = "AreaShape"),
    title = map_chr(
      ss_num,
      ~ str_interp("PCA for Staining Set ${.x}, AreaShape Variables Only")
    ),
    prop_var_explained_plot = map2(
      pca_results, title, make_prop_var_explained_plot
    )
...
We next consider the Intensity variables. For staining sets 1 and 2, around five PCs capture 95\% of the variation. However, for staining set 3, 12-13 PCs are needed.
```{r intensity_pca}
all_data2 %>%
  mutate(
    pca_results = map(data, run_pca, pattern = "Intensity"),
    title = map_chr(
      ss_num,
      ~ str_interp("PCA for Staining Set ${.x}, Intensity Variables Only")
    ),
    prop_var_explained_plot = map2(
      pca_results, title, make_prop_var_explained_plot
    )
...
For each staining set, the top two PCs account for all of the variation.
```{r cycle_pca}
all_data2 %>%
  mutate(
    pca_results = map(data, run_pca, pattern = "Cycle"),
    title = map_chr(
      ss_num,
      ~ str_interp("PCA for Staining Set ${.x}, Cycle Variables Only")
    ),
    prop_var_explained_plot = map2(
      pca_results, title, make_prop_var_explained_plot
    )
...
We next consider the Gated variables, which weren't measured for staining set 1. For staining set 2, all PCs are needed to capture 95\% of the variation. For staining set 3, five PCs are needed.
```{r gated_pca}
all_data2 %>%
  filter(ss_num != 1) %>%
  mutate(
    pca_results = map(data, run_pca, pattern = "Gated"),
    title = map_chr(
      ss_num,
      ~ str_interp("PCA for Staining Set ${.x}, Gated Variables Only")
    ),
    prop_var_explained_plot = map2(
      pca_results, title, make_prop_var_explained_plot
    )
...
The Texture variables were only measured for staining set 2. Six PCs account for 95\% of the variation.
```{r texture_pca}
all_data2 %>%
  filter(ss_num == 2) %>%
  mutate(
    pca_results = map(data, run_pca, pattern = "Texture"),
    title = map_chr(
      ss_num,
      ~ str_interp("PCA for Staining Set ${.x}, Texture Variables Only")
    ),
    prop_var_explained_plot = map2(
      pca_results, title, make_prop_var_explained_plot
    )
...
Looking at the Cells variables, for staining set 1, the first seven PCs account for 95\% of the variation. For staining set 3, the number is nine.
```{r cells_pca}
all_data2 %>%
  filter(ss_num != 2) %>%
  mutate(
    ...
    title = map_chr(
      ss_num,
      ~ str_interp("PCA for Staining Set ${.x}, Cells Variables Only")
    ),
    prop_var_explained_plot = map2(
      pca_results, title, make_prop_var_explained_plot
    )
...
Looking at the Cytoplasm variables, for staining set 1, the first eight PCs account for 95\% of the variation. For staining set 3, the number is 15.
```{r cytoplasm_pca}
all_data2 %>%
  filter(ss_num != 2) %>%
  mutate(
    ...
    title = map_chr(
      ss_num,
      ~ str_interp("PCA for Staining Set ${.x}, Cytoplasm Variables Only")
    ),
    prop_var_explained_plot = map2(
      pca_results, title, make_prop_var_explained_plot
    )
...
Looking at the Nuclei variables, for staining set 1, the first seven PCs account for 95\% of the variation. For staining set 2, the number is around 17. For staining set 3, the number is 11.
```{r nuclei_pca}
all_data2 %>%
  mutate(
    pca_results = map(data, run_pca, pattern = "Nuclei"),
    title = map_chr(
      ss_num,
      ~ str_interp("PCA for Staining Set ${.x}, Nuclei Variables Only")
    ),
    prop_var_explained_plot = map2(
      pca_results, title, make_prop_var_explained_plot
    )
