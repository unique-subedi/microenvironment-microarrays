...
make_heatmap <- function(StainingSet, plate, tbl) {
  tbl %>%
    ...
    facet_wrap(
      vars(Well), nrow = 2, labeller = partial(label_both, sep = " ")
    ) +
    geom_tile() +
    labs(
...
make_outlyingness_plot <- function(ss_num,
                                   plate_num,
                                   plate_data,
                                   plot_type = c("outlyingness", "flag"),
                                   pattern) {
  ...
  if (pattern == ".*") {
    ...
    title_suffix <- str_interp("${pattern} Variables Only")
  }
  ...
  plate_data %>%
    ...
    geom_tile() +
    labs(
      x = "Column Index", y = "Row Index", fill = fill_lab,
      title = str_interp(
        "Staining Set ${ss_num}, Plate ${plate_num}, ${title_suffix}"
      )
    ) +
    theme_bw() +
    theme(plot.title = element_text(hjust = 0.5))
...
make_outlyingness_plots <- function(ss_num,
  ...
  outlyingness_obj <- pca_results %>%
    `$`("x") %>%
    `[`(, seq_len(num_pcs)) %>%
    outlyingness(options = list(type = invariance_type))
  ...
  data <- data %>%
    ...
    mutate(
      outlyingness_plot = map2(
        ...
        ss_num = ss_num, plot_type = plot_type, pattern = pattern
      )
    ) %>%
    select(plate_num, outlyingness_plot)
}
```
...
all_data2 %>%
  ...
  select(!data) %>%
  unnest(outlyingness_plots) %>%
  with(walk(outlyingness_plot, print))
```

```{r areashape_outlyingnesses}
all_data2 %>%
  mutate(
    outlyingness_plots = map2(
      ss_num, data,
...
LI8X00510_Level2 %>%
  ggplot(aes(ArrayColumn, ArrayRow, fill = Spot_PA_SpotCellCount)) +
  facet_wrap(vars(Well), nrow = 2, labeller = partial(label_both, sep = " ")) +
  geom_tile() +
